      ...
      <section style={{ background: "var(--ivory)", borderBottom: "1px solid var(--border)" }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-20 lg:py-28">
          <Reveal className="max-w-3xl">
            ...
            <p className="lede max-w-2xl">A clear overview of what this website does—and does not—collect when you contact PANACHE or continue to third-party services.</p>
            <p className="mt-4 label-caps">Effective July 12, 2026</p>
          </Reveal>
        </div>
      </section>
          ...
          <div className="space-y-5">
            {SECTIONS.map((section, index) => (
              <Reveal key={section.title} delay={index * 0.05} y={24}>
                <article className="p-7 sm:p-9" style={{ background: "oklch(0.995 0.004 80)", border: "1px solid var(--border)" }}>
                  <h2 style={{ color: "var(--charcoal)", fontSize: "clamp(1.35rem, 2.5vw, 1.75rem)", lineHeight: 1.2 }}>{section.title}</h2>
                  <p className="mt-3" style={{ color: "var(--warm-gray)", fontFamily: "var(--font-sans)", fontSize: "1.0625rem", lineHeight: 1.8 }}>{section.text}</p>
                </article>
              </Reveal>
            ))}
          </div>
          <Reveal className="mt-10 text-center" y={20}>
            <p style={{ color: "var(--warm-gray)", fontFamily: "var(--font-sans)", fontSize: "1rem", lineHeight: 1.7 }}>
              Privacy questions or requests may be sent to <a href="mailto:info@panachelashes.com" style={{ color: "var(--rose-gold)", textDecoration: "underline" }}>info@panachelashes.com</a>. For appointment terms, read the <Link href="/policies" style={{ color: "var(--rose-gold)", textDecoration: "underline" }}>Booking Policies</Link>.
            </p>
          </Reveal>
        </div>
      </section>
